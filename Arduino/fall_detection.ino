...
void loop()
{
  ...
  //static int8_t detection = interpreter->output(0)->data.int8[0];
  float detection = interpreter->output(0)->data.f[0];

  Serial.println(detection,6);
 ...
 // TF_LITE_REPORT_ERROR(error_reporter, "Input Array Sum: %f\n",
  //                       summed);
  if (detection > 0.60 && (millis() - previous_time)>1000) {
    digitalWrite(LEDB,LOW);
    digitalWrite(LEDR,HIGH);
    TF_LITE_REPORT_ERROR(error_reporter, "Normal \n");
  }
  
  else if (detection <=0.6)
  {
    previous_time = millis();
    digitalWrite(LEDR,LOW);
    digitalWrite(LEDB,HIGH);
    TF_LITE_REPORT_ERROR(error_reporter, "Fall Detected \n");
  }
